...
export class TcpNatsConnection implements NatsConnection {
  ...
  private async connect(): Promise<void> {
    ...
    this.socket.on("data", (chunk) => this.onData(chunk.toString("utf-8")));
  ...
  private onData(chunk: string): void {
    ...
    for (;;) {
      ...
      if (line.startsWith("MSG ") || line.startsWith("HMSG ")) {
        ...
        const frameRaw = this.buffer.slice(lineEnd + 2, lineEnd + 2 + len);
        this.buffer = this.buffer.slice(lineEnd + 2 + len + 2);
